#!/bin/bash
# Upsert new pre-construction projects using curl + Supabase REST API
set -e
...
MAPBOX="${NEXT_PUBLIC_MAPBOX_TOKEN:?Set NEXT_PUBLIC_MAPBOX_TOKEN env var}"

H1="apikey: $KEY"
H2="Authorization: Bearer $KEY"
H3="Content-Type: application/json"
H4="Prefer: resolution=merge-duplicates"

# Get neighborhood IDs
echo "Fetching neighborhoods..."
HOODS=$(curl -s "$API/neighborhoods?select=id,slug" -H "$H1" -H "$H2")

get_hood_id() {
  echo "$HOODS" | python3 -c "import json,sys; d=json.load(sys.stdin); print(next((x['id'] for x in d if x['slug']=='$1'),''))"
}

# Geocode function
geocode() {
  local addr=$(python3 -c "import urllib.parse; print(urllib.parse.quote('$1'))")
  local result=$(curl -s "https://api.mapbox.com/geocoding/v5/mapbox.places/${addr}.json?access_token=${MAPBOX}&limit=1")
  echo "$result" | python3 -c "
import json,sys
try:
  d=json.load(sys.stdin)
  c=d['features'][0]['center']
  print(f'{c[1]},{c[0]}')
except:
  print('')
"
...
# Generate footprint
gen_footprint() {
  local lat=$1 lng=$2 floors=$3
  python3 -c "
import json
lat,lng,fl = $lat,$lng,$floors
size = min(0.0003, 0.0001 + fl * 0.000003)
hw,hh = size/2, size/2.5
fp = {'type':'Polygon','coordinates':[[[lng-hw,lat-hh],[lng+hw,lat-hh],[lng+hw,lat+hh],[lng-hw,lat+hh],[lng-hw,lat-hh]]]}
print(json.dumps(fp))
"
}
...
ensure_dev() {
  local name="$1"
  local slug=$(echo "$name" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/-/g; s/--*/-/g; s/^-//; s/-$//')
  # Try upsert
  local result=$(curl -s "$API/developers" -H "$H1" -H "$H2" -H "$H3" -H "$H4" \
    -H "Prefer: return=representation,resolution=merge-duplicates" \
    -d "{\"name\":\"$name\",\"slug\":\"$slug\"}" -X POST)
  echo "$result" | python3 -c "import json,sys; d=json.load(sys.stdin); print(d[0]['id'] if isinstance(d,list) and d else '')" 2>/dev/null || echo ""
}

# Upsert project function
upsert_project() {
  local json="$1"
  curl -s "$API/projects" -H "$H1" -H "$H2" -H "$H3" \
    -H "Prefer: return=representation,resolution=merge-duplicates" \
    -d "$json" -X POST | python3 -c "import json,sys; d=json.load(sys.stdin); print('OK' if isinstance(d,list) else d)" 2>/dev/null
}
...
# Check existing
echo "Fetching existing projects..."
EXISTING=$(curl -s "$API/projects?select=slug&limit=500" -H "$H1" -H "$H2")

project_exists() {
  echo "$EXISTING" | python3 -c "import json,sys; d=json.load(sys.stdin); print('yes' if any(x['slug']=='$1' for x in d) else 'no')"
}

# Process each project using Python to drive the loop
python3 << 'PYEOF'
...
def api_post(path, data, extra_headers=None):
    ...
    req = urllib.request.Request(f"{API}/{path}", data=json.dumps(data).encode(), headers=h, method="POST")
    try:
        with urllib.request.urlopen(req) as r:
            return json.loads(r.read())
...
PROJECTS = [
    ...
    {"name":"Una Residences","address":"175 SE 25th Rd, Miami, FL 33129","neighborhood":"brickell","developer":"OKO Group, Cain International","architect":"Adrian Smith + Gordon Gill","floors":47,"totalUnits":135,"priceMin":2900000,"priceMax":21000000,"estCompletion":"2026","status":"NEAR_COMPLETION","category":"ULTRA_LUXURY","description":"Una Residences is a 47-story waterfront tower on Biscayne Bay in Brickell designed by Adrian Smith + Gordon Gill Architecture with 135 expansive residences.","amenities":["Bayfront infinity pool","Private marina","Spa and wellness center","Fitness center","Wine cellar","Children's playroom","Theater","Business center","Sunset lounge"],"unitTypes":"2-5 Bedrooms, Penthouses"},
...
print(f"Found {len(existing_slugs)} existing projects, {len(PROJECTS)} new to process")
...
for p in PROJECTS:
    ...
    if not developer_id:
        ...
        dev_data = api_post("developers", {"name": p["developer"], "slug": dev_slug},
